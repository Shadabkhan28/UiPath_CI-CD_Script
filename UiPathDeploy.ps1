...
#log file
$debugLog = "D:\download_pack\orchestrator-package-deploy.log"
...
$uipathCLI = "D:\download_pack\uipathcli\$cliVersion\lib\net461\uipcli.exe"
if (-not(Test-Path -Path $uipathCLI -PathType Leaf)) {
    WriteLog "UiPath CLI does not exist in this folder. Attempting to download it..."
    try {
        if (-not(Test-Path -Path "D:\download_pack\uipathcli\$cliVersion" -PathType Leaf)){
            New-Item -Path "D:\download_pack\uipathcli\$cliVersion" -ItemType "directory" -Force | Out-Null
        }
        #Download UiPath CLI
[Net.ServicePointManager]::SecurityProtocol = "tls12, tls11, tls"
        Invoke-WebRequest "https://www.myget.org/F/uipath-dev/api/v2/package/UiPath.CLI/$cliVersion" -OutFile "D:\download_pack\\uipathcli\\$cliVersion\\cli.zip";
        Expand-Archive -LiteralPath "D:\download_pack\\uipathcli\\$cliVersion\\cli.zip" -DestinationPath "D:\download_pack\\uipathcli\\$cliVersion";
        WriteLog "UiPath CLI is downloaded and extracted in folder D:\download_pack\uipathcli\\$cliVersion"
        if (-not(Test-Path -Path $uipathCLI -PathType Leaf)) {
